...
def get_overhangL(seqrec,strseq):
    ...
    seqrec: ATGATGATGATG
    strseq: ----TGATGATG
    makes an alignment and culculates the number of - on the left end of strseq in this alignment.
    """
    alignment = pairwise2.align.globalxs(seqrec, strseq,-10,-1,penalize_end_gaps=False)[0]
    overhang=len(alignment[1].split(strseq[0])[0])
    return overhang

def get_overhangR(seqrec,strseq):
    """
    for two seqs
    seqrec: ATGATGATGATGGTGT
    strseq: ----TGATGATG----
    makes an alignment and culculates the number of - on the right end of strseq in this alignment.
    """
    alignment = pairwise2.align.globalxs(seqrec, strseq,-10,-1,penalize_end_gaps=False)[0]
    overhang=len(alignment[1].split(strseq[-1])[-1])
    return overhang

def seq_from_mda(u, segid):
    """
...
def seqrec_from_pdb(pdbid,segid):
    ...
    """
    #There are several ways to do that. Unforunately due to changes in PDB or NCBI API and content they might not work in the future.
    
    #Option zero is to use somehow PDB API directly or through pypdb
    #TODO
    
    #Option one is NCBI 
    #ncbi_record = SeqIO.read(Entrez.efetch(db="protein", id='%s_%s'%(pdbid,segid), rettype="gb", retmode="text"), "genbank")
    #ncbi_record = SeqIO.read(Entrez.efetch(db="nuccore", id='%s_%s'%(pdbid,segid), rettype="gb", retmode="text"), "genbank")
    #This is alos not very stable for big pdbs, which have more than 26 chains.
    
    #Second option is get fasta file from PDB, here is a code snippet that was working (and might still work) before a change to the PDB fasta headers.
    #seqrec={}
    #h=io.StringIO(requests.get('http://www.rcsb.org/pdb/download/viewFastaFiles.do?structureIdList=1KX5&compressionType=uncompressed').content.decode("utf-8") )
    #for record in SeqIO.parse(h,'fasta'):
        #r=record
        #chain=record.id.split(':')[1].split('|')[0]
        #r.id='PDB:1KX5:%s'%chain
        #seqrec[chain]=r
...
#Taken with modification from biopython Bio/Data/SCOPData.py    
residues_3to1 = {
